...
mod tests {
    use crate::{
        ...
        templates::streamingattn::{
            ...
            stream_qkt::{QKTData, QKTDataOut2, QKTOut2, QKT},
            stream_reduce::{ReduceData, ReduceOp, ReduceOpType},
            stream_unary::{UnaryOpType, UnaryScalarData, UnaryScalarOp},
        },
    };
    ...
    fn test_scaled_sm_atten_f32() {
        ...
        let v_iter =
            || (0..(SEQ_LEN_I32 * SEQ_LEN_I32)).map(|_i| Array::from_elem(HEAD_DIM, 1_f32));
        ...
        // Sub
        let (sub_sender, sub_receiver) = parent.bounded::<f32>(short_fifo_chan_size);
        let rowmax_data = BinaryDataOut1::<f32> {
            in1_stream: qkt_long_receiver, // operand 1: N*N result of Q*KT
            in2_stream: rowmax_receiver,   // operand 2: Rowmax of operand 1
            out1_stream: sub_sender,
            latency: 1,
            init_inverval: INIT_INTERVAL,
            inner_loop_bound: SEQ_LEN,
            outer_loop_bound: SEQ_LEN,
        };
        let stream_sub_rowmax = BinaryOpOut1::new(rowmax_data, BinaryOpType::Sub);

        // exp
        let (exp_short_sender, exp_short_receiver) = parent.bounded::<f32>(short_fifo_chan_size);
        let (exp_long_sender, exp_long_receiver) = parent.bounded::<f32>(long_fifo_chan_size);
        let exp_data = UnaryScalarData::<f32> {
            in_stream: sub_receiver,
            out_list: vec![exp_long_sender, exp_short_sender],
            latency: 5,
            init_inverval: INIT_INTERVAL,
            loop_bound: SEQ_LEN * SEQ_LEN,
        };
        let stream_exp = UnaryScalarOp::new(exp_data, UnaryOpType::Exp);

        // RowSum
        let (rowsum_sender, rowsum_receiver) = parent.bounded::<f32>(short_fifo_chan_size);
        let rowsum_data = ReduceData::<f32> {
            in_stream: exp_short_receiver,
            out_stream: rowsum_sender,
            latency: 1,
            init_inverval: INIT_INTERVAL,
            inner_loop_bound: SEQ_LEN,
        ...
        let div_data = BinaryDataOut1::<f32> {
            in1_stream: exp_long_receiver, // operand 1: N*N result of Q*KT
            in2_stream: rowsum_receiver,   // operand 2: Rowmax of operand 1
            out1_stream: div_sender,
            latency: 1,
            init_inverval: INIT_INTERVAL,
        ...
        // Checker
        let out_long_iter = || (0..(SEQ_LEN_I32)).map(|_i| Array::from_elem(HEAD_DIM, 1_f32));
        let qkt_short_checker = CheckerContext::new(out_long_iter, out_receiver);
    ...
    fn test_unscaled_sm_atten_f64() {
        ...
        // Generator
        let (q_sender, q_receiver) =
            parent.bounded::<ArrayBase<OwnedRepr<f64>, Dim<[usize; 1]>>>(short_fifo_chan_size);
        let (kt_sender, kt_receiver) =
            parent.bounded::<ArrayBase<OwnedRepr<f64>, Dim<[usize; 1]>>>(short_fifo_chan_size);
        let (v_sender, v_receiver) =
            parent.bounded::<ArrayBase<OwnedRepr<f64>, Dim<[usize; 1]>>>(short_fifo_chan_size);
        ...
        let q_iter =
            || (0..(SEQ_LEN_I32)).map(|i| Array::from_elem(HEAD_DIM, 0.1 * ((i as f64) + 1.)));
        let kt_iter = || {
            (0..(SEQ_LEN_I32 * SEQ_LEN_I32)).map(|i| {
                if i % SEQ_LEN_I32 == 0 {
                    Array::from_elem(HEAD_DIM, 1.1_f64)
                } else {
                    Array::from_elem(HEAD_DIM, 1_f64)
                }
            })
        };
        let v_iter =
            || (0..(SEQ_LEN_I32 * SEQ_LEN_I32)).map(|_i| Array::from_elem(HEAD_DIM, 1_f64));
        ...
        // QKT block
        let (qkt_sender, qkt_receiver) = parent.bounded::<f64>(short_fifo_chan_size);
        let qkt_data = QKTData::<f64> {
            q: q_receiver,
            kt: kt_receiver,
            out_fifo: vec![qkt_sender],
            latency: 1 + (HEAD_DIM_I32.ilog2() as u64),
            init_inverval: INIT_INTERVAL,
            seq_len: SEQ_LEN,
        };
        ...
        // exp
        let (exp_short_sender, exp_short_receiver) = parent.bounded::<f64>(short_fifo_chan_size);
        let (exp_long_sender, exp_long_receiver) = parent.bounded::<f64>(long_fifo_chan_size);
        let exp_data = UnaryScalarData::<f64> {
            in_stream: qkt_receiver,
            out_list: vec![exp_long_sender, exp_short_sender],
            latency: 5,
            init_inverval: INIT_INTERVAL,
            loop_bound: SEQ_LEN * SEQ_LEN,
        ...
        // RowSum
        let (rowsum_sender, rowsum_receiver) = parent.bounded::<f64>(short_fifo_chan_size);
        let rowsum_data = ReduceData::<f64> {
            in_stream: exp_short_receiver,
            out_stream: rowsum_sender,
            latency: 1,
            init_inverval: INIT_INTERVAL,
        ...
        // Div
        let (div_sender, div_receiver) = parent.bounded::<f64>(short_fifo_chan_size);
        ...
        let (out_sender, out_receiver) =
            parent.bounded::<ArrayBase<OwnedRepr<f64>, Dim<[usize; 1]>>>(short_fifo_chan_size);
        let outer_prod_data = MatmulData::<f64> {
            in_scalar_stream: div_receiver,
            in_vec_stream: v_receiver,
            out_stream: out_sender,
            latency: 1,
